...
- [HOME SHOWCASE](#home-showcase)
	...
	- [Getting Started](#getting-started)
		...
		- [Installing](#installing)
			...
			- [Install Node.js from the EPEL Repository](#install-node.js-from-the-epel-repository)
			- [Install packages from npm](#install-packages-from-npm)
...
* [Postman](https://www.getpostman.com/) - For simulating the incoming requests
...
Set the path for the data directory in the configuration file. For the standard installation it is in
```
/etc/mongodb.conf
```
...
sudo npm install
```
Sometimes npm install will give an error similar to the following
```
npm: relocation error: npm: symbol SSL_set_cert_cb, version libssl.so.10 not defined in file libssl.so.10 with link time reference
```
In that case openssl needs to be updated first
```
...
(Limited by the frontend license)
